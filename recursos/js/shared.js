...
function TabularJson(json) {
    // Ajustar el tamaño de las columnas en base a los nombres de los atributos
    const encabezados = Object.keys(json[0]);
    const worksheet = XLSX.utils.json_to_sheet(json);
    worksheet['!cols'] = encabezados.map(header => ({ wch: header.length + 1 }));
    ...
    // Añadir la hoja de cálculo al libro de trabajo con la fecha en el nombre
    XLSX.utils.book_append_sheet(workbook, worksheet, `Libro_de_Compras_${fechaActual}`);

    // Generar un archivo Excel y descargarlo
    let filenombre = $("#filename").val();
    XLSX.writeFile(workbook, `${filenombre}_${fechaActual}.xlsx`);
}
